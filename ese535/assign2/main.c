...
/* case statement based on approach here */
 ...
 switch (approach)
   ...
   case 1:
     ...
     dummy_tree_place(size,cluster_size);
     break;
   case 2:
     ...
     for (i=1;i<growth_schedule_length;i++)
       growth_schedule[i]=1;
     dummy_tree_place_schedule(growth_schedule,growth_schedule_length,cluster_size);
     break;
   case 3:
     printf("Case 3: Dummy tree placer (C=1, P=0.5).\n");
     growth_schedule=(int *)malloc(growth_schedule_length*sizeof(int));
     growth_schedule[0]=1;
     for (i=1;i<growth_schedule_length;i+=2)
       {
	 growth_schedule[i]=2;
	 if (i+1<growth_schedule_length)
	   growth_schedule[i+1]=1;
       }
     dummy_tree_place_schedule(growth_schedule,growth_schedule_length,cluster_size);
     break;
   case 4:
     printf("Case 4: Dummy tree placer (C=1, P=1).\n");
     growth_schedule=(int *)malloc(growth_schedule_length*sizeof(int));
     growth_schedule[0]=1;
     for (i=1;i<growth_schedule_length;i++)
       growth_schedule[i]=2;
     ...
     break;
   case 5:
     printf("Case 3: Dummy tree placer (C=3, P=0.5).\n");
     growth_schedule=(int *)malloc(growth_schedule_length*sizeof(int));
     growth_schedule[0]=3;
     for (i=1;i<growth_schedule_length;i+=2)
       {
 ...
 if ((unplaced>0) || (precedence_violations>0) || (violations>0) || (makespan_achieved>makespan))
 ...
 else
   {
     ...
     /* route */
     global_route(FALSE); // change to TRUE to make verbose

     // DEBUG
     sprintf(route_tmp_file,"%s.rinfo",blif_file);
     dump_parent_domains(route_tmp_file);

     /* check that route worked */
     int route_fails=valid_routes(FALSE); // change to TRUE to make verbose
     if (route_fails>0)
       {
