 ...
 switch (approach)
   ...
   case 2:
     ...
     for (i=1;i<growth_schedule_length;i++)
       growth_schedule[i]=1;
     dummy_tree_place_schedule(growth_schedule,growth_schedule_length,cluster_size,is_clock,global_clocks);
     break;
   case 3:
     printf("Case 3: Single Partition only, total cut.\n");
     int output;
     output=part_place(size,cluster_size,FALSE,FALSE,FALSE,is_clock,global_clocks);
     calculate_and_write_top_cut(size,cost_file,FALSE,output);
