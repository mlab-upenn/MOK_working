...
int main (int argc, char *argv[]) {
 ...
 char title[] = "\nsched_main version 0.1 for ESE535 Spring 2015\n"
...
/* echo_input (blif_file, lut_size, "packed.echo"); */
 ...
 /* sanity check width, height, cluster_size, input_cluster size vs. netlist */

 if ((width==0)||(height==0))
   {
     int lut_side_lb;
     int io_side_lb;
     printf("Width and height not given, computing.\n");
      
     lut_side_lb=(int)ceil(sqrt((double)((num_blocks-num_p_outputs-num_p_inputs+cluster_size-1)/cluster_size)));
     io_side_lb=(((num_p_outputs+num_p_inputs+input_cluster_size-1)/input_cluster_size)+3)/4;
     height=width=max(lut_side_lb,io_side_lb);

     printf("\theight=width=%d\n",height);
   }

 /* after computing minimum, add extra rows/cols based on pside options */
 width=width+pside;
 height=height+pside;


 total_ios=input_cluster_size*2*(width+height);
 total_luts=cluster_size*width*height;
 printf("Fill Ratios: LUTs=%f IOs=%f\n",
	((double)(num_blocks-num_p_outputs-num_p_inputs)/(double)total_luts),
	((double)(num_p_outputs+num_p_inputs)/(double)total_ios));

 if (((num_blocks-num_p_outputs-num_p_inputs)>total_luts)||
     ((num_p_outputs+num_p_inputs)>total_ios))
   {
     fprintf(stderr,"Array and clustering options too small.\n");
...
/* case statement based on approach here */
 ...
 switch (approach)
   ...
   case 1:
     /* All in one place  */
     printf("Case 1: ASAP Schedule.\n");
     printf("\tShould work, but use excessive number of PEs.\n");
     asap_schedule(width,height,cluster_size,input_cluster_size,makespan);
     break;
   case 2:
     printf("Case 2: Call to your scheduling routine. ");
     /* you may add other arguments as you need them */
     your_schedule(width,height,cluster_size,input_cluster_size,makespan);
     break;
...
/* Parse the command line to determine user options. */
 ...
 if (argc < 6) {
    printf("Usage:  sched_main input.blif output.net output.place architecture.arch output.sched\n");
    printf("\t[-x <int>] [-y <int>] [-pside <int>]\n");
 ...
 while (i < argc) {
   ...
   if (strcmp(argv[i], "\n")==0)
    ...
    if (strcmp (argv[i],"-x") == 0) {
       *width = read_int_option (argc, argv, i);
    ...
    if (strcmp (argv[i],"-y") == 0) {
       *height = read_int_option (argc, argv, i);
