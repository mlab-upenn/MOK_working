...
/* for reporting out cut on the non-recursive partition case,
 ...
 for (i=0;i<num_nets;i++)
  ...
  // For each block on the netlist
    for (int i=0;i<tree_location[tree_location[root].left].used_slots-1;i++)
    {
      int numberOfNets=0;
      int start = 0;

    // Figure out how many nets are connected to the block
      if(block[tree_location[tree_location[root].left].slots[i]].type==1 || block[i].type==3)
      {
        numberOfNets=block[i].num_nets-2;
        start=1;
      }
      else
      {
        numberOfNets=block[i].num_nets;
      }
    ...
    // For all the nets except for the 0th
      for (int j=start; j<numberOfNets; j++)
      {
        ...
        if(mixed_net(block[i].nets[j],left) == TRUE)
        {
