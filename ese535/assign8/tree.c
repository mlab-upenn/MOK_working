...
void allocate_tree_sets_or_not(int *growth_schedule,
			       int growth_schedule_length,
			       int domains,
			       int luts_per_cluster, 
			       int ios_per_iocluster,
			       boolean domains_as_sets)
{
  ...
  for (i=0;i<size;i++)
	...
	for (k=0;k<luts_per_cluster;k++)
	  ...
	  for (j=0;j<growth_schedule[0];j++)
		...
		{
		  tree_location[i].parent_in_domains[j][DOMAIN_LENGTH]=DOMAIN_UNUSED;
		  tree_location[i].parent_out_domains[j][DOMAIN_LENGTH]=DOMAIN_UNUSED;
		}
  ...
  /* switches */
  allocate_subtree(size,0,growth_schedule_length,growth_schedule,total_growth,(int  *)NULL,LEFT,domains,domains_as_sets);
}
...
void allocate_tree(int *growth_schedule,
		   ...
		   int ios_per_iocluster)
{
  allocate_tree_sets_or_not(growth_schedule,growth_schedule_length,domains,luts_per_cluster,ios_per_iocluster,TRUE);
}

void allocate_tree_domains(int *growth_schedule,
		   int growth_schedule_length,
		   int domains,
		   int luts_per_cluster, 
		   int ios_per_iocluster)
{
  allocate_tree_sets_or_not(growth_schedule,growth_schedule_length,domains,luts_per_cluster,ios_per_iocluster,FALSE);
}
   ...
   */
   ...
   switches_top_subtree=(int *)malloc((sizeof(int))*number_switches_at_height);
    ...
    for (i=0;i<number_switches_at_height;i++)
	 ...
	 /* change from assign2 to allocate domains/slots for switches */
	 tree_location[current_node].slots=(int *)malloc(domains*sizeof(int));
	 tree_location[current_node].domain=(int *)malloc(domains*sizeof(int));
	 for (k=0;k<domains;k++)
	   {
	     tree_location[current_node].slots[k]=EMPTY_SLOT;
	     tree_location[current_node].domain[k]=DOMAIN_UNUSED;
 	   }
	 tree_location[current_node].allocated_slots=domains;
	 tree_location[current_node].used_slots=0;
	 /* end change from assign2 */
	 ...
	 if (domains_as_sets==TRUE)
	   ...
	   {
	     tree_location[current_node].left_in_domains[DOMAIN_LENGTH]=DOMAIN_UNUSED;
	     tree_location[current_node].left_out_domains[DOMAIN_LENGTH]=DOMAIN_UNUSED;
	     tree_location[current_node].right_in_domains[DOMAIN_LENGTH]=DOMAIN_UNUSED;
	     tree_location[current_node].right_out_domains[DOMAIN_LENGTH]=DOMAIN_UNUSED;
	   }
	 for (k=1;k<domains;k++)
	   {
	 ...
	 for (j=0;j<growth;j++)
		 ...
		 if (parent_side==LEFT)
		   tree_location[parent_switches[i*growth+j]].left=current_node;
		 else if (parent_side==RIGHT)
		   tree_location[parent_switches[i*growth+j]].right=current_node;
		 else
		   fprintf(stderr,"allocate_subtree: bogus parent side %d at  address %d height %d\n",parent_side,address,height);
	       }
	     else
		 ...
		 tree_location[current_node].parent_out_domains[j][DOMAIN_LENGTH]=domains;
	       ...
	       {
		 tree_location[current_node].parent_in_domains[j][DOMAIN_LENGTH]=DOMAIN_UNUSED;
		 tree_location[current_node].parent_out_domains[j][DOMAIN_LENGTH]=DOMAIN_UNUSED;
	       }
	     for (k=1;k<domains;k++)
	       {
    ...
    /* recurse left */
    right_node_start=allocate_subtree(next_node+number_switches_at_height,address,height-1,growth_schedule,(number_switches_at_height/growth_schedule[height-1]),switches_top_subtree,LEFT,domains,domains_as_sets);

    /* recurse right */
    next_node_start=allocate_subtree(right_node_start,(address+(1<<(height-1))),height-1,growth_schedule,(number_switches_at_height/growth_schedule[height-1]),switches_top_subtree,RIGHT,domains,domains_as_sets);
...
void free_tree_no_block_cleanup()
{
  int i,j;
  for (i=0;i<nodes;i++)
     {
...
/* only use on switches -- to keep track of blocks in subtree */
...

{
  ...
  if (tree_location[loc].node_type!=NODE_TYPE_SWITCH)
    {
      fprintf(stderr,"tree.c: insert_block_switch is intended to only be used with switches, but given node %d which is of type %d\n",
	      loc,tree_location[loc].node_type);
    }
  if (tree_location[loc].slots==(int *)NULL)
    {
      fprintf(stderr,"tree.c: insert_block_switch %d has null slots\n",loc);
      // will then crash...
    }
...
boolean path_to_leaf_side(int tree_loc, int pe_address)
{
  ...
  //printf("PTS: Node address = %d, Node Height =%d, eval = %d\n",node_addr,node_height, node_addr>>node_height);
  //printf("PTS: PE address = %d, Node Height =%d, eval = %d\n",pe_address,node_height, pe_address>>node_height);
  ...
  for (i=0;i<num_blocks;i++)
    {
      ...
      if (loc!=GLOBAL_CLOCK_LOC)
	{
	  int timestep=UNASSIGNED_TIMESTEP; // real timestep will be determined by route
	  // just clearing these out because place_block checks they are cleared
	  block[i].loc=UNPLACED_LOC;
	  block[i].slot_loc=NO_SLOT;
	  place_block(i,loc,timestep);
	}
    }
}

int count_used_parent_domains(int loc, boolean up, int max_domain)
{
  int i;
  int d;
  int res=0;
  for (i=0;i<tree_location[loc].parents;i++)
    {
      if (up==TRUE)
	{
	if (tree_location[i].parent_out_domains!=(int **)NULL)
	  for (d=0;d<max_domain;d++)
	    if (tree_location[loc].parent_out_domains[i][d]!=DOMAIN_UNUSED)
	      res++;
	  }
      else
	{
	if (tree_location[i].parent_in_domains!=(int **)NULL)
	  for (d=0;d<max_domain;d++)
	    if (tree_location[loc].parent_in_domains[i][d]!=DOMAIN_UNUSED)
	      res++;
	  }
    }
...
int *get_used_wires(int height, boolean up, int max_domain)
{
  ...
  for (i=0;i<height;i++)
    res[i]=0;
  int tree_nodes=get_nodes();


  for (i=0;i<tree_nodes;i++)
    {
      res[tree_location[i].height]+=count_used_parent_domains(i,up,max_domain);
    }
  return(res);
}

int max_domain_used(int loc, int max_domain)
{
  int i;
  int d;
  int res=0;
...
int get_max_domain_used(int height, int max_domain)
{
  ...
  for (i=0;i<num_blocks;i++)
    res=max(res,timestep(i));
  // following probably not necessary if output timestep set properly...
  for (i=0;i<tree_size;i++)
    {
      res=max(res,max_domain_used(i,max_domain));
    }
  return(res);
}


void print_detail_domain(FILE *fp, int maxwaves, int *domain)
{
  int i=0;
  for (i=0;i<maxwaves; i++)
    if (domain[i]==DOMAIN_UNUSED)  
      fprintf(fp,"%d: <unused>,",i);
    else
      fprintf(fp,"%d: %s(%d),",i,net[domain[i]].name,domain[i]);
}

void dump_detail_route_domains(char *filename,int maxwaves)
{
 ...
 for(i=0;i<nodes;i++)
   {
     if (tree_location[i].node_type==NODE_TYPE_PE)
       {
	 int used_slots=tree_location[i].used_slots;
	 if (used_slots>0)
	   {
	     fprintf(fp,"PE %d block timesteps ",i);
	     int islot;
	     for (islot=0;islot<used_slots;islot++)
	       {
		 int whichblock=tree_location[i].slots[islot];
		 fprintf (fp,"%d: %s,", tree_location[i].domain[islot], block[whichblock].name);
	       }
	   }
	 fprintf(fp,"\n");
       }
	 ...
	 fprintf(fp,"%d (height=%d, addr=%d) in%d: ",i,tree_location[i].height,tree_location[i].location,j);
	 print_detail_domain(fp,maxwaves,tree_location[i].parent_in_domains[j]);
	 fprintf(fp,"\n");
	 if (tree_location[i].node_type==NODE_TYPE_PE)
	   fprintf(fp,"PE ");
	 else
	   fprintf(fp,"Switch ");
	 fprintf(fp,"%d (height=%d, addr=%d) out%d: ",i,tree_location[i].height,tree_location[i].location,j);
	 print_detail_domain(fp,maxwaves,tree_location[i].parent_out_domains[j]);
	 fprintf(fp,"\n");
       }
     // ALSO INCLUDE right, left for debugging
     if (tree_location[i].left_in_domains!=(int *)NULL)
	 ...
	 if (tree_location[i].node_type==NODE_TYPE_PE)
	   ...
	   fprintf(fp,"Switch ");
	 fprintf(fp,"%d (height=%d, addr=%d) left in: ",i,tree_location[i].height,tree_location[i].location);
	 print_detail_domain(fp,maxwaves,tree_location[i].left_in_domains);
	 fprintf(fp,"\n");
	 if (tree_location[i].node_type==NODE_TYPE_PE)
	   fprintf(fp,"PE ");
	 else
	   fprintf(fp,"Switch ");
	 fprintf(fp,"%d (height=%d, addr=%d) left out: ",i,tree_location[i].height,tree_location[i].location);
	 print_detail_domain(fp,maxwaves,tree_location[i].left_out_domains);
	 fprintf(fp,"\n");
       }
     if (tree_location[i].right_in_domains!=(int *)NULL)
       {
	 if (tree_location[i].node_type==NODE_TYPE_PE)
	   ...
	   fprintf(fp,"Switch ");
	 fprintf(fp,"%d (height=%d, addr=%d) right in: ",i,tree_location[i].height,tree_location[i].location);
	 print_detail_domain(fp,maxwaves,tree_location[i].right_in_domains);
	 fprintf(fp,"\n");
	 if (tree_location[i].node_type==NODE_TYPE_PE)
	   fprintf(fp,"PE ");
	 else
	   fprintf(fp,"Switch ");
	 fprintf(fp,"%d (height=%d, addr=%d) right out: ",i,tree_location[i].height,tree_location[i].location);
	 print_detail_domain(fp,maxwaves,tree_location[i].right_out_domains);
	 fprintf(fp,"\n");
       }
   }
