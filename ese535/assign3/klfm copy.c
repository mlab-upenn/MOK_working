...
void initial_partition(int root, int cluster_size)
{
  ...
  printf("Part size %d used slots %d\n", part_size,tree_location[root+1].used_slots);
  ...
  // First half
    if (i<part_size)
    {
      ...
      printf("Slot %d in tree contains block %s\n", i,block[tree_location[root].left,level[tree_location[root].slots[i]]].name);
  }
  // Second half
    else
...
void repartition(int height, int number_of_blocks, boolean *sub_left, boolean *sub_right)
{
  int i;
  
  //Naively split blocks into two halves 
  int part_size=number_of_blocks>>1;
  printf("Part size %d used slots %d\n", part_size,tree_location[height].used_slots);
   // for (i=0;i<number_of_blocks;i++)
    //recurse_left[i]=FALSE;


  for (i=0;i<tree_location[height].used_slots;i++)
  // First half
    if (i<part_size)
    {
      insert_block_switch(tree_location[height].slots[i],tree_location[height].left,level[tree_location[height].slots[i]]);
      printf("Slot %d in tree contains block %s\n", i,block[tree_location[height].slots[i]].name);
      sub_left[tree_location[height].slots[i]]=1;
      sub_right[tree_location[height].slots[i]]=0;
    }
  // Second half
    else
    {
      insert_block_switch(tree_location[height].slots[i],tree_location[height].right,level[tree_location[height].slots[i]]);
      printf("Slot %d in tree contains block %s\n", i,block[tree_location[height].slots[i]].name);
      sub_left[tree_location[height].slots[i]]=0;
      sub_right[tree_location[height].slots[i]]=1;
    }
}
...
void sub_F_and_T(int height, boolean *sub_left)
{
  int i;
  int j;
  int k;
...
void rearrange_blocks(int switch_num)
{
  ...
  for (i=0;i<left_slots_used;i++)
  {
    ...
    remove_block(tree_location[tree_location[switch_num].left].slots[i], tree_location[switch_num].left);
  ...
  {
    // Remove old placemnt in subtree
    remove_block(tree_location[tree_location[switch_num].right].slots[i], tree_location[switch_num].right);

    #ifdef DEBUG_PART_PLACE
    printf("removed block %s\n",block[tree_location[tree_location[switch_num].right].slots[i]].name);
    #endif
  }
...
void find_PEs(int current_height, struct tree_node *left_switch, struct tree_node *right_switch, int pmax, boolean *is_clock)
{
  if(current_height==0)
  {
    return;
  ...
  else
  {
    ...
    int number_of_blocks=tree_location[tree_location[current_height-1].left].used_slots;
    ...
    int *l_gain = (int *)malloc(number_of_blocks*sizeof(int));
    for (i=0; i<num_blocks; i++)
      l_gain[i]=0;
    ...
    int *sub_left=(int *)malloc(sizeof(int)*number_of_blocks);
    for (i=0; i<num_blocks;i++)
      sub_left[i]=-1;

    // Initialize left array with -1 signifying index is not in this switch
    int *sub_right=(int *)malloc(sizeof(int)*number_of_blocks);
    for (i=0; i<num_blocks;i++)
      sub_right[i]=-1;
    ...
    // Initial partition
    repartition(current_height, number_of_blocks, sub_left, sub_right);
    ...
    l_max_gain_left=initialize_left_branch(is_clock, sub_left, sub_right, current_height, l_gain, l_max_gain_left, pmax, buckets_left, rec);
    l_max_gain_right=initialize_right_branch(is_clock, sub_left, sub_right, current_height, l_gain, l_max_gain_right, pmax, buckets_right,rec);

    /* KLFM */
    int current_cut=calculate_cut(sub_left);
    ...
    while(current_cut<previous_cut && passes<10)
    {
      ...
      while(locked_nodes<(num_blocks-1)&& ((l_max_gain_right>=0 && l_max_gain_right>= l_max_gain_left) || ( l_max_gain_left>=0 && l_max_gain_left>= l_max_gain_right)))
      {
    // Find Max Gain Left
...
void recurse_tree(int height, int location)
{
  if(height==0)
  {
    printf("Got to PE\n");
  ...
  {
    printf("Current Node: location %d, height %d\n",location, height);
    printf("Left Child: location %d, height %d\n",tree_location[location].left, height-1); 
    printf("Right Child: location %d, height %d\n",tree_location[location].right, height-1); 
    printf("\n");
    recurse_tree(height-1, tree_location[location].left);
    recurse_tree(height-1, tree_location[location].right);
    return;
