...
/* new version doesn't treat domains as sets */
...
/* cleanup tree, but leave net placement annotations */
...
/* io in use */
int cluster_io(int loc, int luts_per_cluster, int inputs_per_lut);
...
/* dump out nets assigned to each parent domain -- for global route */
void dump_parent_domains(char *filename);

/* dump nets assigned to domains for global route */
...
/* replace blocks into tree based on placement annotations on blocks */
void replace();

/* count used wires by height */
int *get_used_wires(int height, boolean up, int max_domain);

/* find maximum domain used in tree by looking at leaves */
