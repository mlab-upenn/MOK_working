...
int part_place(int size, int cluster_size, boolean recurse, boolean level_max, boolean verbose, boolean *is_clock, boolean global_clock)
{
  ...
  {
    printf("Block %s\n", block[tree_location[root].slots[i]].name );
  }*/

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Initialization */

  // Function to initialize the partition
  initial_partition(root, cluster_size);

  //
  boolean rec= FALSE;
  
  // Compute the max gain left associated with left branch and fill left buckets etc
  max_gain_left=initialize_left_branch(is_clock, left, right, root, gain, max_gain_left, pmax, buckets_left, rec);
  ...
  // Compute the max gain left associated with left branch and fill left buckets etc  
  max_gain_right=initialize_right_branch(is_clock, left, right, root, gain, max_gain_right, pmax, buckets_right, rec);
  
  #ifdef DEBUG_PART_PLACE
  printf("Initial max gain right: %d\n",max_gain_right);
  #endif

    /*printf("TEST POST ROOT BIPART\n");
  for (i=0;i<tree_location[root].used_slots;i++)
  {
    printf("%s\n",block[tree_location[root].slots[i]].name);
    printf("left = %d\n", block[tree_location[root].slots[i]].left);
  }*/
...
while(current_cut<previous_cut && passes<10)
{
  ...
  while(locked_nodes<(num_blocks-1)&& ((max_gain_right>=0 && max_gain_right>= max_gain_left) || ( max_gain_left>=0 && max_gain_left>= max_gain_right)))
  {
    ...
    current_cut=update_gains_left(active_branch, buckets_left, buckets_right, pmax, max_gain_left, current_cut);
    
    // If active branch is right
    current_cut=update_gains_right(active_branch, buckets_left, buckets_right, pmax, max_gain_right, current_cut);
...
rearrange_blocks(root);



/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//find_PEs(height, tree_location[height].left, tree_location[height].right, pmax, is_clock);
//print_tree();
printf("Recurse on the tree\n");
recurse_tree(tree_location[root+1].height, root+1, pmax, is_clock);
//recurse_tree(tree_location[tree_location[root].right].height,tree_location[root].right, pmax, is_clock);
return(current_cut);
print_tree();
