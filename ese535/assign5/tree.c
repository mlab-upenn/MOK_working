...
void place_block(int whichblock, int loc, int timestep)
{
  ...
  printf("ABOUT TO PLACE BLOCK %s currently at location: %d", block[whichblock].name, block[whichblock].loc);
  #endif
...
void print_node(int i)
{
   int j;
   if (tree_location[i].node_type==NODE_TYPE_PE)
       printf("PE %d used slots %d\n",tree_location[i].location, tree_location[i].used_slots);
