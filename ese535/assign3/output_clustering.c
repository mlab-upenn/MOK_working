 ...
 * to be routed here as inputs.                                     */
    ...
    for (iblk=0;iblk<cluster_occupancy[icluster];iblk++) {
       ...
       // maybe should skip all this on OUTPADs ? AMD 1/17/15
       // output pads don't have their output as pin 0 in the net
       if (block[sub_blk].type!=OUTPAD)
	 {
	   ...
	   if (io_net_mapping[inet] != OPEN) {
	     printf("Error in print_clbs.  Net #%d (%s) in cluster %d at block=%d\n",
		    inet, net[inet].name, icluster,iblk);
	     printf("appears to be driven by two outputs.\n");
	     printf("\tprevious driver is at position %d.\n",io_net_mapping[inet]);
	     exit (1); 
	   }
...
/* Now do the input pins. */
    ...
    for (iblk=0;iblk<cluster_occupancy[icluster];iblk++) {
       ...
       for (ipin=1;ipin<=lut_size;ipin++) {
		...
		/* yes, I see the one below,
		   but look at the next statement.
                     If you wait until the end of the loop,
		      you've long gone off the end of the array.
                      -- andre 2/16/08 */
		if (free_pin_index>inputs_per_cluster)
		  {
		    printf("Error in print_clbs:  cluster %d has too many input pins.\n",
...
/* Now do the clocks. */ 
    ...
    for (iblk=0;iblk<cluster_occupancy[icluster];iblk++) {
       sub_blk = cluster_contents[icluster][iblk];
       if ((block[sub_blk].type==LUT)||(block[sub_blk].type==LUT_AND_LATCH)) // AMD add 1/17/15 since mixing in inputs and outputs
	 {
