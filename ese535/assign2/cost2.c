...
int **max_io_by_height_at_level(int height, int level)
{
  ...
  // Determines parents and height=0 maximum io
    ...
    for (int h=1; h<=max_height-1; h++)
    {
      for (int i=0; i<=get_size(); i++)
      {
        ...
        if(tree_location[parents_for_nav[h-1][i]].parents !=0)
        {
          if( i%2 ==0)
          {
            if(tree_location[*tree_location[parents_for_nav[h-1][i]].parent].left_in_domains != (int*) NULL)
      ...
      // At each node check if we need to update the height maximum
          #ifdef DEBUG
          printf("%d, %d\n", node_max[i], height_max_io_by_level[l][h]);
          #endif

          if (node_max[i]>height_max_io_by_level[l][h] && node_max[i]<32767)
          {
            height_max_io_by_level[l][h]=node_max[i];
    ...
    for (int j=0; j<max_height; j++)
    {
      //printf("Do bad things happen here\n");
      printf("Maximum flow at level %d and height %d: %d\n", l,j, height_max_io_by_level[l][j]);
    }
//printf("Makespan %d\n", level);
