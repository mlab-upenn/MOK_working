...
int update_gains(int active_branch, ListPtr *buckets_left, ListPtr *buckets_right, int pmax, int max_gain_left, int max_gain_right, int current_cut)
{
  ...
  update_case_1(active_branch, whichNet, buckets_left, buckets_right, pmax, max_gain_right, max_gain_left);

  update_case_2(active_branch, whichNet, buckets_left, buckets_right, pmax, max_gain_right, max_gain_left);
  }
  ...
  for(i=0; i<numberOfNets; i++)
  {
    ...
    /*if(active_branch==0)
    {
      whichNet=block[buckets_right[(-max_gain_left)+pmax]->cellNumber].nets[i];
    }

    if(active_branch==1)
    {
      whichNet=block[buckets_left[(-max_gain_right)+pmax]->cellNumber].nets[i];
    }
    printf("which net a: %d\n", whichNet);*/
    ...
    update_case_3(active_branch, whichNet, buckets_left, buckets_right, pmax, max_gain_right, max_gain_left);
    update_case_4(active_branch, whichNet, buckets_left, buckets_right, pmax, max_gain_right, max_gain_left);
  }
...
void update_case_1(int active_branch, int whichNet, ListPtr *buckets_left, ListPtr *buckets_right, int pmax, int max_gain_right, int max_gain_left)
{
  int i;
  int j;
  int k;
...
void update_case_2(int active_branch, int whichNet, ListPtr *buckets_left, ListPtr *buckets_right, int pmax,int max_gain_right, int max_gain_left)
{
  int i;
  int j;
  int k;
...
void update_case_move(int active_branch, int whichBlock, ListPtr *buckets_left, ListPtr *buckets_right, int pmax, int max_gain_right, int max_gain_left, int start, int numberOfNets)
{
  ...
  {
    #ifdef DEBUG_PART_PLACE
    printf("Block for move index %d, name : %s, gain: %d\n", whichBlock, block[whichBlock].name, block[whichBlock].gain);
...
void update_case_3(int active_branch, int whichNet, ListPtr *buckets_left, ListPtr *buckets_right, int pmax, int max_gain_right, int max_gain_left)
{
  int i;
  int j;
  int k;
...
void update_case_4(int active_branch, int whichNet, ListPtr *buckets_left, ListPtr *buckets_right, int pmax, int max_gain_right, int max_gain_left)
{
  int i;
  int j;
  int k;
